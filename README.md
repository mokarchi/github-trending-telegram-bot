...
برای هر ریپو، نام، لینک، زبان، ستاره‌ها، فورک‌ها، منبع کشف و توضیح فارسی ارسال می‌شود. علاوه بر GitHub Trending، بات داستان‌های تازه و برتر Hacker News را هم بررسی می‌کند، لینک‌های GitHub را استخراج می‌کند و ریپوهای تکراری را حذف می‌کند. مقدار `TOP_N` برای هر منبع اعمال می‌شود؛ مقدار پیش‌فرض یعنی تا ۱۰ ریپوی GitHub و تا ۱۰ ریپوی Hacker News در هر دوره. برای Hacker News هیچ Secret جدیدی لازم نیست.
...
```text
TOP_N=10     # برای هر منبع؛ در مجموع تا ۲۰ ریپو
...
Hacker News از API رسمی عمومی خودش خوانده می‌شود. بات فقط داستان‌هایی را که به ریپوی GitHub لینک می‌شوند نگه می‌دارد و اطلاعات ستاره، فورک و زبان را از GitHub تکمیل می‌کند. خروجی هر دوره حداکثر شامل `TOP_N` ریپوی هر منبع است.
